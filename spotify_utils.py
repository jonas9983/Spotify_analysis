...
from use_spotify_api import get_saved_songs, get_user_playlists, get_token, run_api_request, get_track_information
...
def get_playlist(update_dataframe = 0):
    ...
    if (update_dataframe == 1): 
        ...
        playlist_properties_pd = change_panda_variables(flattened_playlist_properties, dance_thresh= 0.6)   
        print(playlist_properties_pd)
        save_json_dataframe(dataframe = playlist_properties_pd, filename = dataframe_path, save = 1)
    else:
...
def get_playlist_tracks(all_playlists, playlists_id, playlist_properties):
    for i, idx  in enumerate(playlists_id):
        ...
        playlist = run_api_request(all_playlists['items'][i]['tracks']['href'], query = "")
        for j in range(playlist['total']):
            if j == 50:
                playlist = run_api_request(all_playlists['items'][i]['tracks']['href'], query= f"?offset={j}")
            genres = get_track_information(playlist['items'][j % 50]['track']['album']['artists'][0]['id'])['genres']
            features['track_name'].append(playlist['items'][j % 50]['track']['name'])
            features['track_artist'].append( playlist['items'][j % 50]['track']['album']['artists'][0]['name'])
...
def change_panda_variables(features, dance_thresh):
    ...
    features['danceability_binary'] = features['danceability'].apply(lambda x: int(1) if x >= dance_thresh else int(0))
    features['key'] = features['key'].apply(lambda x: (
        "C" if x == 0 else 
        "C#" if x == 1 else 
        "D" if x == 2 else 
        "D#" if x == 3 else 
