...
def change_playlist_order(playlist_track_names, folder_track_names, paths, rename=0):
    ...
    missing_track = compare_playlists(
        playlist_1="./sofiii/annie", playlist_2="./sofiii/annie_custom_order"
    )

    if missing_track != []:
        for i, name in enumerate(missing_track["name"]):
            move_to_file(
                folder_track_names=name,
                origin_path=paths,
...
def download_missing_songs(spotify, id, paths, dl=0):
    ...
    for file in os.listdir(paths):
        full_music_name, _, track_name = get_track_properties(
            track=file, playlist=str()
        )
    ...
    missing_tracks, idx = compare_playlists(
        playlist_1=playlist_properties["track_name"], playlist_2=folder_names
    )
    ...
    if dl == 1:
        ...
        os.chdir(paths)
        os.system(
            "spotdl download https://open.spotify.com/playlist/2H1UmImj2tpiz6TIPSu75M"
        )

    change_playlist_order(
        playlist_track_names=playlist_properties["track_name"],
        folder_track_names=music_dict,
        paths=paths,
        rename=0,
    )
...
def compare_playlists(playlist_1, playlist_2):
    ...
    if isinstance(playlist_1, str) and isinstance(playlist_2, str):
        ...
        top = (
            os.listdir(playlist_1)
            if len(os.listdir(playlist_1)) > len(os.listdir(playlist_2))
            else os.listdir(playlist_2)
        )
        bot = (
            os.listdir(playlist_2)
            if len(os.listdir(playlist_1)) > len(os.listdir(playlist_2))
            else os.listdir(playlist_1)
        )
        for track in top:
            if track not in bot:
                full_music_name, artist_name, track_name = get_track_properties(
                    track, playlist=str()
                )
...
def get_track_properties(track=str(), playlist=str()):
    ...
    if track != str():
        ...
        for i in range(playlist["total"]):
            result = get_track_information(
                token, playlist["items"][i]["track"]["album"]["artists"][0]["id"]
            )["genres"]
            track_dict["track_name"].append(playlist["items"][i]["track"]["name"])
            track_dict["track_artist"].append(
                playlist["items"][i]["track"]["album"]["artists"][0]["name"]
            )
            track_dict["track_id"].append(playlist["items"][i]["track"]["id"])
            track_dict["url"].append(
                playlist["items"][i]["track"]["external_urls"]["spotify"]
            )
            if result:
